...
const status = ['Refusé', 'Validé','Annullé']
  ...
  >
    ...
    >
      ...
      <template #item.start_at="{ item }">
        ...
        <small> {{ `${item.raw.issuedDate} 13H:01` }}</small>
      </template>

      <!-- End -->
      <template #item.end_at="{ item }">
        <VIcon icon="tabler-clock-hour-5"/>
        <small>{{ `${item.raw.issuedDate} 17H:01` }}</small>
      </template>

      <!-- Total -->
      <template #item.total="{ item }">
        <VChip color="default" label>
          <VIcon icon="tabler-alarm" />
            3 jrs
        </VChip>
      </template>
      ...
      <template #item.actions="{ item }">
        <IconBtn @click="emit('onAction', item.raw, 'cancel')">
